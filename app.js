...
function toggleFavorite(id){favorites=favorites.includes(id)?favorites.filter(x=>x!==id):[...favorites,id];saveFavorites();renderProducts();toast(favorites.includes(id)?"أضيف للمفضلة":"أزيل من المفضلة");}
...
function renderProducts(){const q=$("searchInput").value.trim().toLowerCase();const filtered=products.filter(p=>(currentCategory==="all"||p.category===currentCategory)&&(!favoritesOnly||favorites.includes(p.id))&&p.name.toLowerCase().includes(q));$("products").innerHTML=filtered.map(p=>`<article class="card">${visualMarkup(p)}<button class="favorite-btn ${favorites.includes(p.id)?"is-favorite":""}" data-favorite="${p.id}" type="button" aria-label="المفضلة">${favorites.includes(p.id)?"♥":"♡"}</button><div class="card-body"><h3>${escapeHtml(p.name)}</h3><div class="price">${money(p.price)}</div><div class="stock-line">${Number(p.stock ?? 0)>0?`متاح: ${Number(p.stock)} قطعة`:`غير متوفر حاليًا`}</div><button class="details-btn" data-details="${p.id}" type="button">عرض التفاصيل</button><button class="add-btn" data-add="${p.id}" type="button" ${Number(p.stock ?? 0)<=0?"disabled":""}>${Number(p.stock ?? 0)<=0?"غير متوفر":"أضف للسلة"}</button></div></article>`).join("");$("resultCount").textContent=`${filtered.length} منتج`;$('emptyState').classList.toggle('hidden',filtered.length!==0);}
...
function addToCart(id){const p=products.find(x=>x.id===id);if(!p)return;const item=cart.find(x=>x.id===id);const nextQty=(item?.qty||0)+1;if(Number(p.stock ?? 0)<nextQty)return toast("الكمية المطلوبة غير متاحة");if(item)item.qty++;else cart.push({...p,qty:1});saveCart();renderCart();toast("تمت إضافة المنتج للسلة");}
...
function openProductDetails(id){const p=products.find(x=>x.id===id);if(!p)return;$("productDetails").innerHTML=`${visualMarkup(p,"detail-emoji")}<div class="detail-content"><h3>${escapeHtml(p.name)}</h3><div class="detail-price">${money(p.price)}</div><div class="stock-line">${Number(p.stock??0)>0?`المتاح: ${Number(p.stock)} قطعة`:`غير متوفر حاليًا`}</div><p>${escapeHtml(p.description||"منتج مختار من متجر لقطة.")}</p><button class="primary-btn" data-detail-add="${p.id}" type="button" ${Number(p.stock??0)<=0?"disabled":""}>${Number(p.stock??0)<=0?"غير متوفر":"🛒 أضف للسلة"}</button></div>`;$("productModal").hidden=false;$("productOverlay").classList.remove("hidden");document.body.classList.add("modal-open");}
...
function openCheckout(){if(!cart.length)return toast("أضف منتجًا إلى السلة أولًا");$("customerName").value=customer.name||"";$("customerPhone").value=customer.phone||"";$("customerAddress").value=customer.address||"";$("checkoutSummary").innerHTML=`<strong>ملخص الطلب</strong>${cart.map(i=>`<div>${escapeHtml(i.name)} × ${i.qty} — ${money(i.qty*i.price)}</div>`).join("")}<hr><strong>الإجمالي: ${money(cartTotal())}</strong>`;$("checkoutModal").hidden=false;$("checkoutOverlay").classList.remove("hidden");document.body.classList.add("modal-open");}
...
async function sendOrder(name,phone,address){customer={name,phone,address};saveCustomer();const order={name,phone,address,items:cart.map(i=>({id:i.id,qty:i.qty}))};
const lines=cart.map(i=>`- ${i.name} × ${i.qty} = ${money(i.qty*i.price)}`);const message=`مرحبًا، أريد تأكيد طلبي من متجر لقطة:\n\nالاسم: ${name}\nالهاتف: ${phone}\nالعنوان: ${address}\n\nالمنتجات:\n${lines.join("\n")}\n\nالإجمالي: ${money(cartTotal())}`;window.open(`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`,"_blank","noopener,noreferrer");try{const saved=await apiRequest("/api/orders",{method:"POST",body:JSON.stringify(order)});orders.unshift(saved);saveOrders();const fresh=await apiRequest("/api/products");if(Array.isArray(fresh)){products=fresh;saveProducts();}}catch(err){toast(err.message);return;}cart=[];saveCart();renderProducts();renderCart();toast("تم حفظ الطلب في قاعدة البيانات وتجهيزه لواتساب");closeCheckout();}
...
$("adminLoginForm")?.addEventListener("submit",e=>{e.preventDefault();if($("adminPin").value===ADMIN_PIN){adminUnlocked=true;closeAdminLogin();renderAdmin();$("adminModal").hidden=false;$("adminOverlay").classList.remove("hidden");document.body.classList.add("modal-open");toast("تم فتح لوحة التحكم");}else{toast("رمز الدخول غير صحيح");}});
...
document.addEventListener("keydown",e=>{if(e.key==="Escape"){closeProductDetails();closeCheckout();closeAdmin();closeSettingsMenu();}});renderProducts();renderCart();
