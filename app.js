...
function changeQty(id,delta){const item=cart.find(x=>x.id===id);if(!item)return;const p=products.find(x=>String(x.id)===String(id));const next=item.qty+delta;if(delta>0&&p&&next>Number(p.stock||0))return toast(`المتاح من ${p.name}: ${Number(p.stock||0)} قطعة`);item.qty=next;if(item.qty<=0)cart=cart.filter(x=>x.id!==id);saveCart();renderCart();}
  ...
  try{
    ...
    if(Array.isArray(remote)){
      ...
      for(const item of cart){
        ...
        if(!live) return toast(`المنتج ${item.name} لم يعد متاحًا`);
        if(Number(live.stock||0)<Number(item.qty||0)) return toast(`المتاح من ${live.name}: ${Number(live.stock||0)} قطعة فقط`);
        item.price=effectivePrice(live); item.stock=Number(live.stock||0);
      }
