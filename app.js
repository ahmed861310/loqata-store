document.addEventListener("click",e=>{const delCoupon=e.target.closest("[data-delete-coupon]");if(delCoupon){apiRequest(`/api/admin/coupons/${delCoupon.dataset.deleteCoupon}`,{method:"DELETE"}).then(()=>{adminCoupons=adminCoupons.filter(c=>c.id!==delCoupon.dataset.deleteCoupon);renderAdmin();toast("تم حذف الكوبون");}).catch(err=>toast(err.message));}});
...
const NOTIFICATION_TYPES={customer:[['order_created','طلب جديد للعميل'],['order_status','تغيير حالة الطلب'],['shipping_update','تحديث الشحن'],['payment_update','تحديث الدفع']],admin:[['new_order','طلب جديد للإدارة'],['order_status','تغيير حالة الطلب'],['shipping_update','تحديث الشحن'],['payment_update','تحديث الدفع']]};
...
async function loadShippingSettings(){try{shippingSettings=await apiRequest("/api/shipping-settings");const sel=$("shippingZone");if(sel){sel.innerHTML=(shippingSettings.zones||[]).map(z=>`<option value="${escapeHtml(z.id)}">${escapeHtml(z.name)} — ${money(z.fee)}</option>`).join("");}}catch(e){shippingSettings={zones:[],freeShippingThreshold:1000};}}
...
الإجمالي النهائي: ${money(saved.total)}`;window.open(`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`,'_blank','noopener,noreferrer');cart=[];saveCart();renderProducts();renderCart();toast('تم حفظ الطلب وتجهيزه لواتساب');closeCheckout();}catch(err){toast(err.message);}}
...
$("checkoutForm").addEventListener("submit",async e=>{e.preventDefault();const name=$("customerName").value.trim(),phone=$("customerPhone").value.trim(),address=$("customerAddress").value.trim(),couponCode=$("couponCode")?.value.trim().toUpperCase()||"";if(!name||!phone||!address||!$("shippingZone").value)return toast("من فضلك أكمل بيانات الطلب واختر منطقة التوصيل");otpState={challengeId:"",phone,name,address,couponCode,paymentMethod:document.querySelector("input[name=paymentMethod]:checked")?.value||"cod"};try{const r=await apiRequest("/api/otp/request",{method:"POST",body:JSON.stringify({phone})});otpState.challengeId=r.challengeId;$("checkoutModal").hidden=true;$("otpModal").hidden=false;$("otpOverlay").classList.remove("hidden");$("otpCode").value="";$("otpMessage").textContent=r.devCode?`وضع التطوير: رمز التحقق ${r.devCode}`:"تم إرسال رمز التحقق عبر SMS";document.body.classList.add("modal-open");$("otpCode").focus();}catch(err){toast(err.message);}});
