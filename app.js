...
async function openCheckout(){const storedCart=readJson("loqataCart",cart);if(Array.isArray(storedCart))cart=storedCart;renderCart();if(!cart.length)return toast("أضف منتجًا إلى السلة أولًا");await loadShippingSettings();if(authenticatedUser){try{const profile=await apiRequest('/api/profile');const u=profile.user||{};const local=getSavedCustomer();customer={name:local.name||u.name||'',phone:local.phone||u.phone||'',address:local.address||u.address||''};saveCustomer();}catch{}}selectedCoupon=null;$("couponCode").value="";if($("redeemPoints"))$("redeemPoints").value=0;loadLoyalty();$("couponMessage").textContent="";fillCheckoutFromSavedCustomer();updateCheckoutSummary();$("checkoutModal").hidden=false;$("checkoutOverlay").classList.remove("hidden");document.body.classList.add("modal-open");}
...
async function openCustomerAccount(){
  ...
  try{
    ...
    customer={name:savedCustomer.name||authenticatedUser.name||'',phone:savedCustomer.phone||authenticatedUser.phone||'',address:savedCustomer.address||authenticatedUser.address||''};
    saveCustomer();
...
$("accountForm")?.addEventListener('submit',async e=>{e.preventDefault();customer={name:$('accountName').value.trim(),phone:$('accountPhone').value.trim(),address:$('accountAddress').value.trim()};saveCustomer();try{if(authenticatedUser){const r=await apiRequest('/api/profile',{method:'PUT',body:JSON.stringify(customer)});authenticatedUser=r.user||authenticatedUser;}}catch(err){console.warn('تعذر حفظ الملف الشخصي على الخادم',err.message);}fillCheckoutFromSavedCustomer();toast('تم حفظ البيانات وستظهر تلقائيًا في الطلب');});
