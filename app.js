...
function formatOrderDate(value){
  ...
  let m=raw.match(/^(\d{1,2})T(\d{2}):(\d{2}):\d{2}(?:\.\d+)?Z-(\d{1,2})-(\d{4})$/);
  if(m){
    const h=Number(m[2]),hh=((h+11)%12)+1,ampm=h<12?"ص":"م";
    return `${m[1].padStart(2,"0")}/${m[4].padStart(2,"0")}/${m[5]} - ${String(hh).padStart(2,"0")}:${m[3]} ${ampm}`;
  }
  const d=new Date(raw);
  if(Number.isNaN(d.getTime()))return raw;
  return new Intl.DateTimeFormat("ar-EG",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:true}).format(d);
}
...
let campaigns = [];
let adminCampaigns = [];
let checkoutState = {paymentMethod:"cod"};
...
function addToCart(id){const key=String(id);const p=products.find(x=>String(x.id)===key);if(!p)return toast("تعذر العثور على المنتج. حدّث الصفحة وحاول مرة أخرى");const item=cart.find(x=>String(x.id)===key);const nextQty=(item?.qty||0)+1;if(Number(p.stock ?? 0)<nextQty)return toast("الكمية المطلوبة غير متاحة");if(item){item.price=effectivePrice(p);item.qty++;}else cart.push({...p,price:effectivePrice(p),qty:1});saveCart();renderCart();toast("تمت إضافة المنتج للسلة");}
function changeQty(id,delta){const key=String(id);const item=cart.find(x=>String(x.id)===key);if(!item)return;const p=products.find(x=>String(x.id)===key);const next=item.qty+delta;if(delta>0&&p&&next>Number(p.stock||0))return toast(`المتاح من ${p.name}: ${Number(p.stock||0)} قطعة`);item.qty=next;if(item.qty<=0)cart=cart.filter(x=>String(x.id)!==key);saveCart();renderCart();}
...
document.addEventListener("click",e=>{const fav=e.target.closest("[data-favorite]");if(fav){toggleFavorite(fav.dataset.favorite);return;}const add=e.target.closest("[data-add]");if(add){addToCart(add.dataset.add);return;}const details=e.target.closest("[data-details]");if(details){openProductDetails(details.dataset.details);return;}const detailAdd=e.target.closest("[data-detail-add]");if(detailAdd){const id=detailAdd.dataset.detailAdd;const qty=Math.max(1,Number($("detailQty")?.value||1));for(let i=0;i<qty;i++)addToCart(id);closeProductDetails();return;}const detailBuy=e.target.closest("[data-detail-buy]");if(detailBuy){const id=detailBuy.dataset.detailBuy;const qty=Math.max(1,Number($("detailQty")?.value||1));for(let i=0;i<qty;i++)addToCart(id);closeProductDetails();openCart();return;}const plus=e.target.closest("[data-plus]");if(plus){changeQty(plus.dataset.plus,1);return;}const minus=e.target.closest("[data-minus]");if(minus){changeQty(minus.dataset.minus,-1);return;}const remove=e.target.closest("[data-remove]");if(remove){cart=cart.filter(x=>String(x.id)!==String(remove.dataset.remove));saveCart();renderCart();return;}const setting=e.target.closest("[data-setting]");if(setting){handleSetting(setting.dataset.setting);return;}const favFilter=e.target.closest("[data-favorites-filter]");if(favFilter){favoritesOnly=!favoritesOnly;favFilter.classList.toggle("active",favoritesOnly);renderProducts();return;}const cat=e.target.closest(".cat");if(cat){document.querySelectorAll(".cat").forEach(b=>b.classList.remove("active"));cat.classList.add("active");currentCategory=cat.dataset.category;renderProducts();return;}const saveProduct=e.target.closest("[data-save-product]");if(saveProduct){const id=Number(saveProduct.dataset.saveProduct);const p=products.find(x=>String(x.id)===String(id));if(!p)return;document.querySelectorAll(`[data-product-id="${id}"]`).forEach(el=>{if(el.dataset.field==='price')p.price=Math.max(0,Number(el.value)||0);else if(el.dataset.field==='salePrice')p.salePrice=el.value?Math.max(0,Number(el.value)):null;else if(el.dataset.field==='stock')p.stock=Math.max(0,Number(el.value)||0);else if(el.dataset.field)p[el.dataset.field]=el.value.trim();});p.name=p.name||'منتج بدون اسم';p.emoji=p.emoji||'🛍️';apiRequest(`/api/admin/products/${id}`,{method:'PATCH',body:JSON.stringify(p)}).then(saved=>{products=products.map(x=>String(x.id)===String(id)?saved:x);cart=cart.map(i=>String(i.id)===String(id)?{...i,...saved,price:effectivePrice(saved)}:i);saveProducts();saveCart();renderProducts();renderCart();renderAdmin();toast('تم حفظ تفاصيل المنتج في قاعدة البيانات');}).catch(err=>toast(err.message));return;}const del=e.target.closest("[data-delete-product]");if(del){const id=Number(del.dataset.deleteProduct);apiRequest(`/api/admin/products/${id}`,{method:'DELETE'}).then(()=>{products=products.filter(p=>String(p.id)!==String(id));cart=cart.filter(i=>String(i.id)!==String(id));saveProducts();saveCart();renderProducts();renderCart();renderAdmin();toast('تم حذف المنتج من قاعدة البيانات');}).catch(err=>toast(err.message));return;}const delOrder=e.target.closest('[data-delete-order]');if(delOrder){const id=Number(delOrder.dataset.deleteOrder);apiRequest(`/api/admin/orders/${id}`,{method:'DELETE'}).then(()=>{orders=orders.filter(o=>String(o.id)!==String(id));saveOrders();renderAdmin();toast('تم حذف الطلب من قاعدة البيانات');}).catch(err=>toast(err.message));return;}});
...
(async function syncProductsFromServer(){
  try{
    ...
    products=remote;
    const byId=new Map(remote.map(p=>[String(p.id),p]));
    cart=cart.filter(item=>byId.has(String(item.id))).map(item=>{
      const live=byId.get(String(item.id));
      const stock=Math.max(0,Number(live.stock||0));
      return {...item,...live,qty:Math.min(Math.max(1,Number(item.qty||1)),stock),price:effectivePrice(live),stock};
...
async function openCustomerAccount(){
  ...
  $("accountOrders").innerHTML=mine.length?`<h3>طلباتي السابقة</h3>${mine.map(o=>`<details class="order-card"><summary>#${escapeHtml(o.id)} — ${money(o.total)} — ${escapeHtml(o.status||'جديد')}</summary><p>${escapeHtml(formatOrderDate(o.date))}<br>${(o.items||[]).map(i=>`${escapeHtml(i.name)} × ${i.qty}`).join('<br>')}</p></details>`).join('')}`:'<p class="empty">لا توجد طلبات محفوظة حتى الآن.</p>';
