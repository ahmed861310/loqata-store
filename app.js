...
function formatOrderDate(value){
  ...
  if(Number.isNaN(d.getTime())){
    const m=raw.match(/^(\d{1,2})T(\d{2}):(\d{2}):(\d{2})(?:\.(\d{1,3}))?Z-(\d{1,2})-(\d{4})$/);
    if(m)d=new Date(Date.UTC(Number(m[7]),Number(m[6])-1,Number(m[1]),Number(m[2]),Number(m[3]),Number(m[4]),Number((m[5]||"0").padEnd(3,"0"))));
  }
  if(Number.isNaN(d.getTime()))return raw;
  return new Intl.DateTimeFormat("ar-EG",{timeZone:"Africa/Cairo",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:true}).format(d);
}
...
function renderAdminOrderDetails(data){
  ...
  $("adminOrderDetails").innerHTML=`
    <div class="order-detail-grid">
      <div class="order-detail-card"><h3>📦 بيانات الطلب</h3><p><strong>رقم الطلب:</strong> #${escapeHtml(o.id)}</p><p><strong>التاريخ:</strong> ${escapeHtml(formatOrderDate(o.date))}</p><p><strong>العميل:</strong> ${escapeHtml(o.name||"")}</p><p><strong>الهاتف:</strong> ${escapeHtml(o.phone||"")}</p><p><strong>العنوان:</strong> ${escapeHtml(o.address||"")}</p></div>
