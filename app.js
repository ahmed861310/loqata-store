...
async function openCheckout(){if(!cart.length)return toast("أضف منتجًا إلى السلة أولًا");await loadShippingSettings();selectedCoupon=null;$("couponCode").value="";if($("redeemPoints"))$("redeemPoints").value=0;loadLoyalty();$("couponMessage").textContent="";$("customerName").value=customer.name||"";$("customerPhone").value=customer.phone||"";$("customerAddress").value=customer.address||"";updateCheckoutSummary();$("checkoutModal").hidden=false;$("checkoutOverlay").classList.remove("hidden");document.body.classList.add("modal-open");}
