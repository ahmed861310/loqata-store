...
const readDb = () => { const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!db.shipping) db.shipping=DEFAULT_SHIPPING; if(!Array.isArray(db.notifications)) db.notifications=[]; return db; };
const addNotification = (db, data) => { db.notifications.push({id:id('ntf'), read:false, createdAt:new Date().toISOString(), ...data}); if(db.notifications.length>2000) db.notifications=db.notifications.slice(-2000); };
...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url==='/api/orders'){
    ...
    const couponCode=String(b.couponCode||'').trim().toUpperCase(); const coupon=(db.coupons||[]).find(c=>c.code===couponCode&&c.active!==false); const byId=new Map(db.products.map(p=>[String(p.id),p])); let subtotal=0; const items=[];
  ...
  if(req.method==='PATCH'&&url.startsWith('/api/admin/orders/')&&url.endsWith('/details')){
    ...
    if(b.status && String(b.status)!==String(o.status)){
      ...
      o.timeline.push({id:id('evt'),type:'status',title:`تغيير الحالة: ${o.status}`,note:`من: ${from} ← إلى: ${o.status}`,date:now,actorName:'مدير المتجر'});if(o.userId)addNotification(db,{userId:o.userId,role:'customer',type:'order_status',orderId:o.id,title:'تحديث حالة الطلب',message:`الطلب #${o.id}: الحالة الآن ${o.status}.`});addNotification(db,{role:'admin',type:'order_status',orderId:o.id,title:'تغيير حالة طلب',message:`الطلب #${o.id}: ${from} ← ${o.status}.`});
    }
    if(b.paymentStatus!==undefined && String(b.paymentStatus)!==String(o.paymentStatus||'')){
      ...
      if(o.userId)addNotification(db,{userId:o.userId,role:'customer',type:'payment_update',orderId:o.id,title:'تحديث الدفع',message:`تم تحديث حالة دفع الطلب #${o.id} إلى ${o.paymentStatus}.`});
      addNotification(db,{role:'admin',type:'payment_update',orderId:o.id,title:'تحديث دفع',message:`تم تحديث دفع الطلب #${o.id} إلى ${o.paymentStatus}.`});
  ...
  if(req.method==='PATCH'&&url.startsWith('/api/admin/orders/')){const orderId=url.split('/').pop();const b=await body(req);const db=readDb();const o=db.orders.find(x=>String(x.id)===orderId);if(!o)return json(res,404,{error:'الطلب غير موجود'});if(b.status && !ORDER_STATUSES.has(String(b.status)))return json(res,400,{error:'حالة الطلب غير صالحة'});if(b.status){const oldStatus=o.status;o.status=String(b.status);o.statusHistory=Array.isArray(o.statusHistory)?o.statusHistory:[];const now=new Date().toISOString();o.statusHistory.push({status:o.status,date:now});if(o.userId&&oldStatus!==o.status)addNotification(db,{userId:o.userId,role:'customer',type:'order_status',orderId:o.id,title:'تحديث حالة الطلب',message:`الطلب #${o.id}: الحالة الآن ${o.status}.`});addNotification(db,{role:'admin',type:'order_status',orderId:o.id,title:'تغيير حالة طلب',message:`الطلب #${o.id}: ${oldStatus||'غير محدد'} ← ${o.status}.`});}writeDb(db);return json(res,200,o);}
