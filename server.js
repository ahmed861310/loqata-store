...
const readDb = () => { const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!db.shipping) db.shipping=DEFAULT_SHIPPING; if(!Array.isArray(db.notifications)) db.notifications=[]; if(!Array.isArray(db.campaigns)) db.campaigns=[]; if(!Array.isArray(db.reviews)) db.reviews=[]; if(!Array.isArray(db.questions)) db.questions=[]; if(!db.notificationSettings) db.notificationSettings=DEFAULT_NOTIFICATION_SETTINGS; if(!db.notificationTemplates) db.notificationTemplates=DEFAULT_NOTIFICATION_TEMPLATES; if(!db.loyalty) db.loyalty={config:{...LOYALTY_CONFIG},balances:{}}; if(!db.loyalty.config) db.loyalty.config={...LOYALTY_CONFIG}; if(!db.loyalty.balances) db.loyalty.balances={}; return db; };
