...
if (!fs.existsSync(DB_FILE)) {
  fs.writeFileSync(DB_FILE, JSON.stringify({ users: [], products: [], orders: [], coupons: [], shipping: DEFAULT_SHIPPING, otpChallenges: [], notifications: [], campaigns: [], reviews: [] }, null, 2));
}
const readDb = () => { const db=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); if(!db.shipping) db.shipping=DEFAULT_SHIPPING; if(!Array.isArray(db.notifications)) db.notifications=[]; if(!Array.isArray(db.campaigns)) db.campaigns=[]; if(!Array.isArray(db.reviews)) db.reviews=[]; if(!db.notificationSettings) db.notificationSettings=DEFAULT_NOTIFICATION_SETTINGS; if(!db.notificationTemplates) db.notificationTemplates=DEFAULT_NOTIFICATION_TEMPLATES; if(!db.loyalty) db.loyalty={config:{...LOYALTY_CONFIG},balances:{}}; if(!db.loyalty.config) db.loyalty.config={...LOYALTY_CONFIG}; if(!db.loyalty.balances) db.loyalty.balances={}; return db; };
...
if(req.method==='GET'&&url==='/api/admin/reviews'){const db=readDb();return json(res,200,(db.reviews||[]).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)));}
  ...
  if(req.method==='GET'&&url==='/api/products'){
    ...
    return json(res,200,db.products.map(p=>{const x=stats.get(String(p.id))||{sum:0,count:0};return {...p,ratingCount:x.count,ratingAverage:x.count?Math.round((x.sum/x.count)*10)/10:0};}));
  }
