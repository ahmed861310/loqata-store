...
if(req.method==='GET'&&url==='/api/admin/reviews'){const db=readDb();return json(res,200,(db.reviews||[]).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)));}
  ...
  if(req.method==='PATCH'&&url.startsWith('/api/admin/orders/')){const orderId=url.split('/').pop();const b=await body(req);const db=readDb();const o=db.orders.find(x=>String(x.id)===orderId);if(!o)return json(res,404,{error:'الطلب غير موجود'});if(b.status && !ORDER_STATUSES.has(String(b.status)))return json(res,400,{error:'حالة الطلب غير صالحة'});if(b.status){const oldStatus=o.status;o.status=String(b.status);awardLoyalty(db,o);o.statusHistory=Array.isArray(o.statusHistory)?o.statusHistory:[];const now=new Date().toISOString();o.statusHistory.push({status:o.status,date:now});if(o.userId&&oldStatus!==o.status)addNotification(db,{userId:o.userId,role:'customer',type:'order_status',orderId:o.id,vars:{orderId:o.id,status:o.status}});addNotification(db,{role:'admin',type:'order_status',orderId:o.id,title:'تغيير حالة طلب',message:`الطلب #${o.id}: ${oldStatus||'غير محدد'} ← ${o.status}.`,vars:{orderId:o.id,status:o.status}});}writeDb(db);return json(res,200,o);}
