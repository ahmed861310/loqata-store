...
if(req.method==='GET'&&url==='/api/admin/reviews'){const db=readDb();return json(res,200,(db.reviews||[]).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)));}
  ...
  if(req.method==='PUT' && url==='/api/profile'){const u=currentUser(req);if(!u)return json(res,401,{error:'يجب تسجيل الدخول'});const b=await body(req);u.name=String(b.name||u.name||'').trim();u.phone=String(b.phone||'').trim();u.address=String(b.address||'').trim();writeDb(db);return json(res,200,{user:publicUser(u)});}
...
function publicUser(u){return {id:u.id,name:u.name,email:u.email,role:u.role,phone:u.phone||'',address:u.address||'',createdAt:u.createdAt};}
